...
const main = async () => {
    ...
    let files = await getFiles().then((res) => {
        return res
    })    
    ...
    let downloadRequestedFiles = requested.map((item) => {
        console.log("\nDownloading .gitignore for " + item.split(".")[0])
        return downloadFile(files[item]).then((data) => {
            gitIgnoreContent += `# GET IGNORE file for ${item.split(".")[0]}\n\n${data}\n\n`
        })
    })

    Promise.all(downloadRequestedFiles).then(() => {
        fs.writeFile(".gitignore-f", gitIgnoreContent, function(err) {
            if(err) {
                console.log("\n" + err);
            }

            console.log("\nYour .gitignore was sucessfully generated!");
        }); 
    })
